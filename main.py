...
• ЗАЧЕМ-СТРОКА (необязательна, максимум одна на пункт): отдельной строкой сразу после ссылки, с префиксом «↳ », одно предложение до 15 слов — что azalio с этим сделать или что для него меняется: «↳ Обнови Cilium до 1.20, если Gateway API у тебя с внешней авторизацией», «↳ Закрывает ручное ведение values.schema.json в чартах». Это единственное место для «зачем»; внутри факта хвостов по-прежнему нет. Нечего сказать конкретно — строку не пиши. «Полезно знать», «стоит следить», «это важно для…», «это сигнал, что…» — запрещённые формы и для этой строки.
...
• Дописывать в конце факта (внутри самого буллета) объясняющий или оценочный хвост, где подлежащее — «это», «для X это», «специалистам/командам», а не новый факт. Под запрет попадают все разновидности:
  ...
  Правило: пункт = факт(ы) + ссылка (+ необязательная ↳ зачем-строка). Никакого редакторского хвоста внутри факта о том, что он значит; ↳ строка подчиняется тем же ⛔ — только конкретное действие или изменение для azalio. Короткий конкретный практический смысл внутри факта («дешевле inference», «убирает простой при выкатке») — это НЕ хвост, его писать нужно.
