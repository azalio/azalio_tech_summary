...
• ВЫГОДНЫЙ ДОСТУП К LLM — приоритетная для читателя тема (он кодит с AI и платит за инференс). ВКЛЮЧАЙ конкретные предложения по цене/доступу:
  — скидки, промо, бесплатные кредиты/тиры, заметное снижение цен на модели и API (токены/инференс) у любого провайдера;
  — дешёвый или единый доступ к моделям: подписки, агрегаторы/шлюзы/прокси к моделям (в духе ClinePass, OpenRouter), бандлы, доступ к топ-моделям за низкую цену, доступ без отдельных API-ключей;
  — появление open-weight моделей в общем/дешёвом доступе или их удешевление;
  — AI-кодинг-ассистенты и агентные CLI (Cline, Cursor, Aider, Claude Code, Codex, GitHub Copilot, Continue), их значимые релизы/апдейты — особенно меняющие цену или доступ к моделям.
  Это прямое действие: подключить более дешёвый доступ, сменить тариф/инструмент. Такой конкретный анонс цены/скидки/доступа — это НЕ «мелкий анонс» из правила 1, его ВКЛЮЧАЙ; отсекай только чистый хайп/фандрейзинг без реальной выгоды по цене или доступу.
  ✅ «ClinePass: подписка для доступа из Cline к open-weight моделям (GLM, Kimi, Deepseek, Minimax) без отдельных API-ключей, для части моделей — со скидкой 2–5×» — берём: дешёвый доступ к моделям.
  ✅ «Провайдер снизил цену токенов модели на 40%» / «бесплатные кредиты на инференс open-weight модели» — берём: прямая выгода по стоимости инференса.
