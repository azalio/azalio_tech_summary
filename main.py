...
ПРОПУСКНЫЕ ВОРОТА — бери пункт, если он попадает хотя бы в одни:
...
• ОФИЦИАЛЬНЫЕ РЕЛИЗЫ моделей, API, кодинг-агентов и инструментов (Cline, Cursor, Aider, Claude Code, Codex, Copilot, Continue) и значимые изменения доступности или цены.
...
• КОНКРЕТНЫЕ ЭКСПЕРТНЫЕ НАХОДКИ с кодом, данными, замерами, постмортемом — то, что можно перенести к себе сразу.
...
• реклама, вакансии, конференции и вебинары, советы уровня «как писать промпты», общие мнения и колонки;
...
• ЛИТМУС-ТЕСТ для сомнительных: «azalio сделает с этим что-то на этой неделе — сменит тариф, обновит стек, изменит конфиг, пересчитает бюджет GPU, попробует инструмент?» Либо «это настолько неожиданно, что он перескажет это коллеге?» Нет ни того, ни другого → выбрасывай.
...
🔬 — любопытное исследование или эксперимент, а не рабочий инструмент
...
• «До 5», а не «ровно 5». Два сильных пункта лучше пяти с наполнителем.
...
• Дописывать в конце пункта объясняющий или оценочный хвост, где подлежащее — «это», «для X это», «специалистам/командам», а не новый факт. Под запрет попадают все разновидности:
  ...
  Правило: пункт = факт(ы) + ссылка. Никакого редакторского хвоста о том, что факт значит. Короткий конкретный практический смысл внутри факта («дешевле inference», «убирает простой при выкатке») — это НЕ хвост, его писать нужно.
